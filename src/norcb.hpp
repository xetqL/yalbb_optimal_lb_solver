...
struct NoRCB {
    ...
    inline Real distance(const Stripe& a, const Stripe& b) const {
        // minimum of the distance between
    }
    ...
    friend NoRCB* allocate_from(NoRCB& t);
};

NoRCB* allocate_from(NoRCB& t) {
    auto* ptr = new NoRCB(t.comm);
